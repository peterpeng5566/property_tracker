...
(function (root) {
  ...
  function restoreFromBackup(data, backupId, opts = {}) {
    ...
    // Self-protection entry: snapshot of the CURRENT state, with the
    // current state recorded as a backup. The caller (Alpine shim) can
    // apply this directly to data.backups OR via the returned
    // selfProtectionEntry. The new data.backups array already includes
    // this entry (pushBackup semantics with FIFO 5).
  ...
  async function writePortfolioBackupFile(fileId, content, opts) {
  ...
  async function listPortfolioBackupFiles(fileId, opts) {
    if (!opts || typeof opts.fetchFn !== 'function') {
  ...
  async function cleanupOldBackups(fileId, keep = 5, opts) {
    if (!opts || typeof opts.fetchFn !== 'function') {
      throw new Error('cleanupOldBackups: opts.fetchFn is required');
    }
    const { fetchFn } = opts;
    const files = await listPortfolioBackupFiles(fileId, { fetchFn });
  ...
  const api = { buildBackupSnapshot, pushBackup, restoreFromBackup, writePortfolioBackupFile, listPortfolioBackupFiles, cleanupOldBackups };
