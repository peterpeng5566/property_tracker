...
(function (root) {
  ...
  function mergeSettings(local, remote) {
    // v1.7 — Object-level newer-wins on `settings.updated_at`
    // (ADR 0016, supersedes ADR 0009 §5's replace-from-remote
    // limitation). Settings is a singleton object, not
    // record-bearing — no deletion-log, no per-record merge.
    // Strict `>` so tie → local (symmetric with mergeById /
    // mergeByIdWithDeletions tie-break). Missing / invalid
    // `updated_at` treated as epoch 0 (mirror mergeById tsOf
    // semantics).
  ...
  function mergePortfolios(local, remote, deviceId) {
    // v1.7 — supersede ADR 0009 §5's replace-from-remote v1
    // limitation for both categories and settings:
    //   * categories: per-record `mergeByIdWithDeletions` (mirror
    //     plans / holdings / cash / debts / snapshots pattern,
    //     including deletion-log propagation — ADR 0011 type enum
    //     amended to include 'categories').
    //   * settings: object-level newer-wins via mergeSettings
    //     (whole object, no per-field timestamps).
    const settings = mergeSettings(local.settings, remote.settings);
    const categories = mergeByIdWithDeletions(local.categories, remote.categories, local.deletions, remote.deletions);
  ...
  const api = { mergeById, mergeByIdWithDeletions, mergeSettings, mergePortfolios };
