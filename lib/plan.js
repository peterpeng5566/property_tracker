...
(function (root) {
  ...
  function newRule() {
    ...
    return { id: _genId('rule'), name: '', when: {}, distribute: {} };
  }
  ...
  // Validate one rule. `allCategories` is accepted for future use
  // (e.g., surface category-id typos) but the current contract does NOT
  // require category ids to exist — that check belongs in the UI layer
  // so a user can save a plan referencing a category they just deleted.
  // Delete-protection flows through plansReferencingCategory instead.
  function validateRule(rule, allCategories) {
    const errors = [];
    ...
    // allCategories is accepted but unused for now — see header comment.
    void allCategories;
  ...
  //
  // `categories` is accepted for future use (e.g., look up target category
  // display names) but is unused in the current contract.
  function driftForRule(rule, records, recordsAttributes, categories, fxRate) {
    void categories;
    ...
    if (filteredTotal === 0) {
      ...
      // to drift against the target distribution.
  ...
  function driftForPlan(plan, records, recordsAttributes, categories, fxRate) {
    if (!_isPlainObject(plan) || !Array.isArray(plan.rules)) return [];
    return plan.rules.map(rule => driftForRule(rule, records, recordsAttributes, categories, fxRate));
  }
