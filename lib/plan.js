...
(function (root) {
  ...
  //
  function driftForRule(rule, records, recordsAttributes, fxRate) {
  ...
  function driftForPlan(plan, records, recordsAttributes, fxRate) {
    if (!_isPlainObject(plan) || !Array.isArray(plan.rules)) return [];
    return plan.rules.map(rule => driftForRule(rule, records, recordsAttributes, fxRate));
  }
