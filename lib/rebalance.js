// lib/rebalance.js — pure rebalance advisor (v1.8).
//
...
//
// Source of truth for the v1.8 rebalance advisor (ADR 0017):
//   - Each Plan rule carries an optional `target_weight_pct` field
//     (additive, schema stays at '1.1' per ADR 0009 §6).
//   - When set, the rule is "rebalance-eligible": the lib computes a
//     per-rule target_value = total_portfolio_value × target_weight_pct,
...
(function (root) {
  ...
  // A rule is rebalance-eligible iff it has a finite target_weight_pct
  // in [0, 100]. Missing / null / undefined → not eligible (drift-only).
