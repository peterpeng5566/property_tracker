...
(function (root) {
  ...
  //   fetchFn: optional function(url) => Promise<Response>. Defaults to
  //            global fetch in browser. Tests inject a mock.
  //
  // Returns Promise<{ [symbol: string]: Quote | { failed: true } }>
  //   On total input error (no fetchFn available, symbols not array): throws YahooAuthError.
  ...
  async function fetchQuotes(symbols, fetchFn) {
    ...
    const results = await Promise.all(unique.map(s => fetchOne(s, fn)));
    const out = {};
