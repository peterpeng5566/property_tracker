...
(function (root) {
  ...
  // toDisplaySeries: pure helper for the snapshot sparkline (v1.5 T05).
  // Takes the raw data.snapshots[] and returns one entry per snapshot
  // with netWorth + holdingsValue reconverted to the *current* display
  // currency. Each snapshot uses its own frozen fx_rate (NOT the current
  // one) so historical values stay anchored to when they were captured.
  // Sorted ascending by date so the chart's left→right axis reads as
  // time progression (oldest → newest).
  function toDisplaySeries(snapshots, currentDisplayCurrency) {
    if (!Array.isArray(snapshots)) return [];
    const target = currentDisplayCurrency || 'TWD';
    // Defensive copy + sort; never mutate the caller's array.
    const sorted = [...snapshots]
      .slice()
      .sort((a, b) => String(a.date || '').localeCompare(String(b.date || '')));
    return sorted.map(snap => {
      const srcCurrency = snap.totals?.displayCurrency || 'TWD';
      const fxRate = (typeof snap.fx_rate === 'number' && Number.isFinite(snap.fx_rate))
        ? snap.fx_rate
        : 1;
      return {
        id: snap.id,
        date: snap.date,
        netWorth: Format.convertCurrency(snap.totals?.netWorth ?? 0, srcCurrency, target, fxRate),
        holdingsValue: Format.convertCurrency(snap.totals?.holdingsValue ?? 0, srcCurrency, target, fxRate),
      };
    });
  }
