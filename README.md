...
- **v1.15** — Refresh completion toast. Pressing Refresh no longer returns a silent path: success now fires a 5-second emerald toast (`Refreshed N holdings`), partial fires an amber warning toast (`Refreshed X of Y holdings, M failed`) while the existing amber `Retry N failed` button stays persistent, and the rose error banner is unchanged for proxy-misconfig / full-failure. Cancel and 0-active-holdings remain silent. The fire point is `_applyRefreshResult` so the retry path automatically reports the **retry scope** (not the full portfolio). Reuses the existing `window.__toast` bridge (`portfolio.html:6263`) — no new UI surface, no new toast component. No schema change. `lib/` unchanged. 140 → 146 browser smoke (`tests/browser/refresh-toast.spec.js`, T1–T6). See [`docs/adr/0022-refresh-toast.md`](docs/adr/0022-refresh-toast.md) and `.scratch/v1.15-refresh-toast/issues/01-refresh-toast.md`.
- **v1.16** — Refresh writes through to sync. `_applyRefreshResult` now bumps `holdings.updated_at` (and stamps `device_id`) on each successful Yahoo fetch, so the v1.12 auto-pull mechanism finally propagates the freshest price across devices instead of silently merging ties in favour of whichever device opened last. Failed fetches leave both fields untouched (in-memory `_refresh_failed` flag still flips; per ADR 0009 §4 stripped at `save()`). The fix is two lines inside the existing shim — `lib/` untouched, no schema bump, no new UI surface, no new i18n keys. `CONTEXT.md` gains a canonical `## Refresh` glossary entry pinning the term vs *Manual edit* / *Snapshot* / *Backup*. 146 → 147 browser smoke (`tests/browser/v17-sync.spec.js` gains a `v1.16: refresh stamp trigger` sibling test). Known limitation: pre-v1.16 portfolios can have holdings stuck at their original cost-edit `updated_at`; first manual Refresh on each holding after upgrading heals the divergence. See [`docs/adr/0023-refresh-is-an-edit.md`](docs/adr/0023-refresh-is-an-edit.md) and `.scratch/v1.16-refresh-is-an-edit/issues/01-refresh-is-an-edit.md`.
- **v1.17** — Home Plan vs Actual now shows amount-based drift alongside the existing percentage drift. Each rule's target amount = `netWorth × target_weight_pct` (where missing = 100%); each sub-item's target amount = `rule.target_amount × distribute_weight[value_id]`. Seven columns on the desktop table (`value | target% | actual% | delta% | target$ | actual$ | delta$`); rule card header gains `Matching | Target | Δ`; section header gains `Σ target` + an over-100% warning. Display currency follows `settings.displayCurrency`. Delta color is shared between `delta%` and `delta$` (5pp threshold). Negative `actual$` for debt records is preserved verbatim. Math lives in `lib/plan.js` (extended `driftForRule` return shape); `portfolio.html` is the Alpine shim. See [`docs/adr/0024-home-plan-amounts.md`](docs/adr/0024-home-plan-amounts.md).
...
_Last updated at v1.17 close-out._ The remaining open items are in the per-version issue trackers under `.scratch/`:
...
- `.scratch/v1.15-refresh-toast/issues/01-refresh-toast.md` — v1.15 ships with 1 resolved ticket (refresh completion feedback: success toast / partial warning toast + persistent amber Retry button / error banner unchanged / cancel silent). Fire point in `_applyRefreshResult` so retry path reports retry scope. 6 new browser tests (T1 all-success success toast; T2 partial warning toast + amber button + row badge; T3 proxy-misconfig rose banner + no toast; T4 cancel mid-flight silent; T5 0 active holdings silent; T6 retry path toast reports retry scope, not full portfolio). Reuses existing `window.__toast` bridge — no new UI surface, no schema, `lib/`, or new dependency change.
- `.scratch/v1.16-refresh-is-an-edit/issues/01-refresh-is-an-edit.md` — v1.16 ships with 1 resolved ticket (refresh writes through to sync: `_applyRefreshResult` bumps `holdings.updated_at` + stamps `device_id` on each successful fetch, failed fetches leave both untouched). Two-line change inside the existing shim; `lib/` untouched; no schema bump; no UI / i18n change. ADR 0023 codifies the design choice (Refresh is an edit for sync semantics) and rejects the alternatives (separate `refreshed_at` field; refresh-only sync channel; backfill migration). Known limitation: pre-v1.16 portfolios may carry holdings stuck at the original cost-edit `updated_at` — first manual Refresh after upgrading heals the divergence. 1 new browser test (sibling to the v1.7 settings stamp-trigger test in `tests/browser/v17-sync.spec.js`): refresh DOES bump `holdings.updated_at` + `device_id` on success, MUST NOT bump on failed fetches, MUST NOT touch non-target tickers.
- `.scratch/v1.17-home-plan-amounts/issues/01-...` — v1.17 ships with 3 resolved tickets (data layer + ADR 0024; UI + i18n + browser integration; docs + close-out). Extends `driftForRule` with `target_amount` / `actual_amount` / `drift_amount` / `rule_target_amount` (all in baseline TWD). ADR 0024 codifies 6 design decisions (net-worth basis, treat-missing-as-100, display currency, shared `drift_class`, debt-negative semantics, total-target warning). 5 new browser scenarios; no schema bump; no new field. Glossary gains 3 new terms under `## Plans`.

## Docs

- [CONTEXT.md](CONTEXT.md) — domain glossary
- [docs/adr/](docs/adr/) — architectural decisions (0001–0024)
