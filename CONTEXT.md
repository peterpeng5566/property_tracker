...
**Candidate action**:
A single (rule, record) row on the Rebalance page showing the per-holding or per-cash-account advice for one rebalance-eligible rule's matched records. The per-row `Target $` equals the record's bucket target (= `rule_target × distribute_weight[value_id] / 100`, same number on every row in the same bucket); the per-row `Δ $` equals the bucket delta (`bucket_target − bucket_current`, NOT divided by bucket count) — both back-converted to the record's native currency for display. The user picks which record(s) within a bucket to actually execute; each row's Buy/Sell shares are computed using *that* row's `current_price` so the user can choose any row and the per-share math stays correct. Records missing the distribute attribute land in a synthetic `_unassigned` bucket with weight 0 (target 0, delta = −current). All candidates are independent rows; when 2+ rules match the same holding, the page shows N separate rows (one per rule), each in the respective rule's section. ADR 0026.
_Avoid_: Trade order, suggestion, recommendation

**Rebalance target value**:
The total value (`rule.target_weight_pct × portfolio.totalValue`, in baseline currency) that one rebalance-eligible rule is aiming to allocate. v1.20 (ADR 0026) splits this across distribute buckets via `rule_target × distribute_weight[value_id] / 100`, and each per-row candidate cell shows the *bucket* target (= Home's `target_amount[value_id]`, same number on every row in the same bucket) NOT divided by bucket record count. Section summary lines show the whole-rule target in baseline currency; per-candidate-row cells show the record's native currency (per ADR 0017 §6 — "act vs measure"). Holding candidates' `target_shares = bucket_target_native / current_price`; cash candidates' `target_balance_native = bucket_target_native`.
_Avoid_: Allocation amount, target dollar amount, even-split (v1.8 term, removed)
